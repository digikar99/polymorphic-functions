...
;; Suppose we have a polymorph for coercing to a type denoted by type-specifier TS1, and another polymorph for coercing to a type denoted by type-specifier TS2. Suppose TS2 is a subtype of TS1.
;; - Now, if asked to coerce to TS1, it is perfectly valid to coerce to TS2. In fact, the polymorph corresponding to TS2 can be called for any specifier that is a *supertype* of TS2. Similarly, the polymorph corresponding to TS1 can be called for any specifier that is a *supertype* of TS1.
;; - In addition, the set of type specifiers that are supertypes of TS1 is a subset of the set of type specifiers that are supertypes of TS2. In other words, (subtypep '(supertypep TS1) '(supertypep TS2)) is T, which means dispatching on the most specialized polymorph is the same as dispatching on the polymorph corresponding to TS1. And this is consistent with what @Commander_Trashdin  just said.
